...
/**
 * Types of configuration parameters passed into oe_create_enclave
 */
typedef enum _oe_config_type
{
    OE_CONFIG_TYPE_SWITCHLESS,
} oe_config_type_t;

/**
 * Base structure type for configuration parameters.
 */
typedef struct _oe_config
{
    oe_config_type_t type;
    size_t size;
    struct _oe_config* next;
} oe_config_t;

/**
 * The number of worker threads for switchless calls.
 */
typedef struct _oe_config_switchless
{
    /* Extends oe_config_t. */
    oe_config_t base;
    size_t num_host_workers;
} oe_config_switchless_t;
 ...
 *
 * @param configs List of additional enclave creation configurations for
 * the specific enclave type.
 *
 * @param config_count The number of the configurations in the list.
 *
